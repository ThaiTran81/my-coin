...
export const SendTransactionModal = (props) => {
    ...
    const handleSendTransaction = () => {
        ...
        axios.post('http://localhost:9080/transactions', newTx).then((res) => {
        ...
        <div className='send-transaction-modal'>
            <div className={`confirm ${(step === 1)? '': 'confirm--hidden'}`}>
                ...
                <div className='content'>
                    ...
                    <div className='content__amount'>Amount: <span className='txt--main'>MC</span><span className='txt--warning'>{numeral(transaction.amount).format('0,0')}</span></div>
                    <div className='content__fee'>Transaction fee: <span className='txt-right'>0.20 $</span></div>
                </div>
                <div className='bottom'>
                    <p className="confirm__notice">*Please check your transaction again.</p>
                    <div className='confirm__btn' onClick={handleSendTransaction}>{(send)? <i class="fa fa-spinner fa-pulse fa-fw"></i> : 'Send'}</div>
                </div>
            </div>
