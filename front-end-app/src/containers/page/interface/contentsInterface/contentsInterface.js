...
export const ContentsInterface = (props) => {
    ...
    return (
        <div className='contents-interface'>
            <Modal state={modalState} onClickOverlay={() => {
                setModalState(enumState.CLOSE)
            }}>
                {(() => {
                    switch (optionModal) {
                        case enumOptionModal.WALLET:
                            return <WalletModal closeModal={() => {
                                setupModal()
                            }}></WalletModal>;
                        case enumOptionModal.LOGOUT:
                            return <LogoutModal closeModal={() => {
                                setupModal()
                            }}></LogoutModal>;
                        case enumOptionModal.SEND:
                            return <SendTransactionModal setTransaction={setNewTx} blockchain={blockchain}
                                                         transaction={newTx}
                                                         setupModal={setupModal}></SendTransactionModal>;
                        default:
                            break;
                    }
                })()}
            ...
            <div className='top-content'>
                <div className='content-item item--color-main'>
                    <div className='content-item__image' style={{
...

                {
                    (() => {
                        switch (option) {
                            ...
                            case 2:
                                return <SendTransaction transaction={newTx} setTransaction={setNewTx}
                                                        walletBalance={balance} openModal={() => {
                                    setupModal(enumState.VISIBLE, enumOptionModal.SEND)
                                }}></SendTransaction>
                            case 3:
                                return <PendingTransaction pendingTxs={blockchain.getPendingTransactionsOutput()}
                                                           pendingTx={pendingTx} onMining={() => {
                                    mineNewBlock(blockchain.getPendingTransactions(), blockchain.getLastBlock().index+1, new Date().toString(), blockchain.getLastBlock().hash);
                                }
                                }/>

                            case 4:
                                return <HistoryTransaction stateMine={stateMine}
                                                           pendingTx={pendingTx}></HistoryTransaction>
                            case 5:
                                return <HistoryLog/>
                            default:
                                break;
