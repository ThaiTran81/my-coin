...
export const HistoryTransaction = ({pendingTx, stateMine}) => {
    ...
    useEffect(() => {
        const fetchDate = async () => {
            const result = await axios.get(`http://localhost:9080/transactions/${myWallet.publicKey.substring(2, myWallet.publicKey.length)}`);
                            ...
                            tableData.length > 0 && tableData.map((item, index) => {
                                return (
                                    <tr key={index}>
                                        ...
                                        <td>{(+item.block !== -1)? new Date(item.timeStamp).toLocaleString() : 'Waiting...'}</td>
