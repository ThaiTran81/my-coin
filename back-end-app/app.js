...
app.get('/transactions', (req, res) => {
    return res.status(200).json(blockchain.getTransactions().sort(function(x, y){
...
app.post('/blocks/mining', (req,res) => {
    if (res.body !== undefined && res.body.length > 0) {
        setTimeout(() => {
            const lastBlock = blockchain.getLastBlock();
            io.emit(socketEvt.START_MINING, ({index: lastBlock.index+1,transactions: res.body, timeStamp: new Date().toString(), prevHash: lastBlock.hash}));
        }, 1000);
    }
    blockchain.setMine(true);
    res.json({ message: 'add transaction success' }).end();
